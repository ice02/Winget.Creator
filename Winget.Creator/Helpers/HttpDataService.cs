...
namespace Winget.Creator.Helpers
{
    ...
    public class HttpDataService
    {
        ...
        public async Task<List<T>> GetAsyncAsList<T>(string uri, string accessToken = null, bool forceRefresh = false) where T : class
        {
            ...
            if (forceRefresh || !responseCache.ContainsKey(uri))
            {
                ...
                try
                {
                    var res = await Task.Run(() => JsonConvert.DeserializeObject<SearchApiResponse<T>>(json));
                    result = new List<T> { res.Data };

                }
                catch (Exception)
                {
                    var res = await Task.Run(() => JsonConvert.DeserializeObject<SearchApiResponse<List<T>>>(json));
                    result = res.Data;
                }
        ...
        public async Task<HttpResponseMessage> PostAsJsonWithResponseAsync<T>(string uri, T item)
        {
            if (item == null)
            {
                return null;
